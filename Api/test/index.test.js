...
describe("Test de RUTAS", ()=>{
        describe( 'GET /rickandmorty/character/:id',()=>{
            ...
            it( 'Si hay un error responde con status: 500',async() =>{
                await agent.get('/rickandmorty/character/4565').expect(500);
            })

        })
        describe("GET /rickandmorty/login",()=>{
            it('La informacion de Login es correcta',async()=>{
                const response = (await agent.get('/rickandmorty/login?email=pepito@hotmail.com&password=123456a')).body
                expect(response.access).toEqual(true)
            })

            it('La informacion de Login es incorrecta',async()=>{
                const response = (await agent.get('/rickandmorty/login?email=pedrito@hotmail.com&password=123')).body
        ...
        describe("POST /rickandmorty/fav",()=>{
            const character={id:"1",name:"joel"}
            const character2={id:"2",name:"joel2"}
        ...
        describe("DELETE /rickandmorty/fav/:id",()=>{
            ...
            it('Devuelve el arreglo correspondiente si no se elimina ningun personaje',async()=>{
                const response = (await agent.delete('/rickandmorty/fav/4856')).body
                expect(response).toContainEqual(character)
                expect(response).toContainEqual(character2)
